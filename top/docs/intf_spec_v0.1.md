...
日期：2026-08-30
依据文档：`ma_spec_v0.1.md`（§1.2–§1.7、§2–§11）、`isa_spec_v0.1.md`（§1.7）。
适用范围：本文档定义 easy_simt SIMT 处理器 v1 的**模块级接口**：端口命名规则、vld/rdy 握手协议、全部模块间通道的信号级定义、`memif` 对外 AXI4 总线约定与顶层信号。本文档是全套文档端口定义的唯一源（单源规则见 §1），是各模块级规范端口章节与 `top` 连线的直接依据。
勘误记录：2026-08-30 `sf_ialu_issue` 载荷增设 `opc` 向量（R3 型 IMAD 第三源，8×32b；见 `top/cmodel/sim_common.h` 偏差 C5，§2）。
修订记录：2026-08-30 结构改进：前置元数据补依据文档；§1 增设端口表单源规则声明；§1.9 一致性自查固化为固定清单。内容定义不变。

---

## 1. 总体约定

**端口表单源规则**：本文档 §2–§11 的端口表是全套文档端口定义的唯一源；模块级规范引用本文档章节号，不复制端口表；模块特有的增设端口与行为约束在模块级规范内标注"模块补充"。
...
5. 位宽参数（§1.4）与 ma_spec §1.6 参数总表一致。
6. 术语与 ma_spec §1.4 术语对照一致。
7. 模块级规范的端口章节引用本文档章节号，不复制端口表；模块增设端口标注"模块补充"。
8. 模块交付时，本文档对应章节端口表与该模块 RTL 端口清单一一对应。
