...
# -*- coding: utf-8 -*-
"""
easy_simt_assembler_verify.py — 汇编器端到端验证程序（内含功能级 ISS）。
...
说明：
  * 完整实现 BR/JOIN + BRT 分化处理语义（见 ma_spec §8）；纯标准库
...
# ---------------- 编码常量（与 easy_simt_assembler.py 保持一致） ----------------
...
import easy_simt_assembler


def cpu_reference(in_bits, n, block=256, mask=0x80):
    """与 easy_simt_kernel.cu 的 cpu_reference 同构（MASK 参数化），
...
def main():
    ...
    ap.add_argument('--ptx', default=None,
                    help='PTX 源（经 easy_simt_assembler 汇编）')
    ...
    ap.add_argument('--prog', default=None,
                    help='直接装载已汇编镜像（easy_simt_kernel.json）运行 ISS')
    ...
    if args.prog:
        print('== 1. 装载已汇编镜像 %s ==' % args.prog)
        with open(args.prog, 'r', encoding='utf-8') as f:
            prog = json.load(f)
    else:
        if not args.ptx:
            sys.exit('需要 --ptx 或 --prog（或 --selftest）')
        print('== 1. 汇编 PTX -> easy_simt ISA ==')
        prog, _ = easy_simt_assembler.assemble_file(args.ptx)
    m = prog['meta']
    ...
    if args.outdir:
        ...
        with open(os.path.join(args.outdir, 'easy_simt_kernel.json'), 'w') as f:
