...
`tmp/sram/<名>/`（已生成则跳过）。需要宏的模块在 `make rtl` / `make netlist` /
`make syn` 前均自动检查宏产物，缺失先执行 `make sram`：

- **仿真**：用宏的行为模型（`<名>.v`，负沿读写仿真视图；Makefile 现场生成
  关闭 `$display` 的仿真变体参与编译）；
- **综合**：宏按黑盒处理，例化保留于网表（物理宏口径）；面积取宏的 Liberty，
  综合结尾自动汇总「逻辑 + 宏实例数 × 宏面积」。
