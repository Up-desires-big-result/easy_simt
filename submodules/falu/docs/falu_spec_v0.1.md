...
修订记录：2026-08-31 初版，结构沿用模块级规范骨架（bs_spec/ialu_spec 先例）。
...
1. 对齐：取 `q = max(A.q, B.q)`，`da = q − A.q`，`db = q − B.q`；构造定量整数 `X = A.mant << db`、`Y = B.mant << da`（左对齐，无精度损失），则两操作数之和差为 `(X op Y) × 2^(q − da − db)`；
2. 符号相同：`M = X + Y`，结果符号取 `A.sg`；符号相异：按幅值比较，`X = Y` 时精确抵消、结果 +0；否则 `M` 为大幅值减小幅值（301 位定量整数），结果符号取大幅值操作数的符号；
3. 单舍入（RN，对 `M × 2^(q − da − db)`，与 `rne_big` 一致）：设 `lz` 为 `M` 最高置位位。`lz ≥ 23` 时右移 `lz − 23` 位，取 24 位尾数域（bit23 为 1），`guard`/`sticky` 同 §5.3 条 3，舍入进位至 2^24 时右移一位、指数加 1，得 `qq = (q − da − db) + (lz − 23)`（含进位加 1）；`lz < 23` 时左移 `23 − lz` 位对齐至 bit23，无舍入，`qq = (q − da − db) − (23 − lz)`；
4. 打包：同 §5.3 条 4（`e = qq + 150`，`e ≥ 255` → ±∞，`e ≤ 0` → FTZ 带符号 ±0，否则 `{sgn, e[7:0], mant[22:0]}`）。

对齐移位量最大为 `q` 域全幅差（Denorm 最小 `q = −172` 与 Normal 最大 `q = 104`，差 276），`X`/`Y` 取 24 + 276 = 300 位、`M` 取 301 位，保证精确求和后再单次舍入（与 `softfloat.c` 的 320 位定量整数等价）。
